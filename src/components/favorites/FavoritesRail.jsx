...
export default function FavoritesRail({ favorites, onPlay }) {
  ...
  if (favoriteChannels.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-white/[0.03] rounded-2xl border border-[var(--hairline)] border-dashed m-4">
        <p className="text-[#A1A1AA] font-medium">No favorites yet.</p>
        <p className="text-[#6B6B73] text-sm mt-1">Press ☆ on any channel to save it here.</p>
      </div>
    );
  }
    ...
    <div className="flex overflow-x-auto no-scrollbar scroll-smooth gap-4 p-4">
      {favoriteChannels.map(channel => (
        <button
          ...
          className="u-tile u-focus group relative flex-shrink-0 w-64 h-36 rounded-xl overflow-hidden text-left"
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black via-black/80 to-transparent">
            <div className="flex justify-between items-end">
              <div className="flex-1 truncate pr-2">
                ...
                <p className="text-xs text-[#E8B15A] font-medium truncate drop-shadow-md">{channel.groups?.[0]}</p>
              </div>
