    ...
    <div className="p-4 bg-[#123236] rounded-xl border border-gray-700">
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          disabled={!dirty}
          className="px-4 py-2 rounded-lg text-sm font-medium transition-colors bg-blue-600 hover:bg-blue-500 text-white disabled:opacity-40 disabled:cursor-not-allowed"
        >
          Save
        </button>
      </div>
...
function ShortcutsSettings() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-[#123236] rounded-xl border border-gray-700 divide-y divide-gray-700/60">
        {shortcuts.map(([key, desc]) => (
          <div key={key} className="flex items-center justify-between px-4 py-3">
            <span className="text-sm text-gray-300">{desc}</span>
            <kbd className="px-2.5 py-1 bg-[#0a1f22] border border-gray-600 rounded-md text-xs font-mono text-blue-300 whitespace-nowrap ml-4">
              {key}
            </kbd>
          </div>
        ))}
