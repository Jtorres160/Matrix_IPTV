...
export default function BottomNavigationBar({ activeZone, onSelect, currentView }) {
  return (
    <nav
      className="fixed bottom-0 left-0 w-full u-glass border-t border-white/10 z-50 flex justify-around items-center h-16 px-2 md:hidden"
    >
      {MENU_ITEMS.map((item, index) => {
        ...
        return (
          <button
            ...
            className={`relative flex flex-col items-center justify-center w-full h-full space-y-1 transition-colors ${isActive ? 'text-[#F0C27B]' : 'text-[#8a8a93] hover:text-white'}`}
          >
            {isActive && <span className="absolute top-0 h-0.5 w-8 rounded-full bg-[#E8B15A] shadow-[0_0_12px_rgba(232,177,90,0.7)]" />}
            <item.icon size={22} className={isActive ? 'text-[#F0C27B]' : 'text-current'} />
