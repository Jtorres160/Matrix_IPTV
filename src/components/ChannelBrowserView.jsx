...
export default function ChannelBrowserView() {
  ...
  return (
    <div className="flex h-full w-full bg-[#0a1214] text-gray-200">

      {/* ── LEFT: categories ── */}
      <div className="w-72 shrink-0 border-r border-white/10 flex flex-col bg-[#0c171a]">
        <div className="p-3 border-b border-white/10">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-black/40 border border-white/10 rounded-lg py-2 pl-8 pr-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-teal-500"
            />
          </div>
        </div>
      ...
      <div className="flex-1 min-w-0 border-r border-white/10 flex flex-col">
        <div className="p-3 border-b border-white/10 flex items-center gap-3">
          ...
          <span className="text-xs text-gray-500 shrink-0">{listChannels.length} channels</span>
        </div>
        ...
        <div className="p-4 pb-2">
          ...
          {activeChannel && (
            <button
              ...
              className={`mt-3 w-full flex items-center justify-center gap-2 py-2.5 bg-teal-600 hover:bg-teal-500 text-white rounded-lg font-semibold text-sm transition-colors focus:outline-none ${
                pane === PANE.DETAIL ? 'ring-2 ring-teal-300 ring-offset-2 ring-offset-[#0a1214]' : ''
              }`}
            >
              ...
              <div className="mt-1 mb-3">
                ...
                {programProgress(nowProg) != null && (
                  <div className="w-full h-1 bg-white/10 rounded-full mt-2 overflow-hidden">
                    <div className="h-full bg-teal-500 rounded-full" style={{ width: `${Math.round(programProgress(nowProg) * 100)}%` }} />
                  </div>
                )}
              </div>
            )}
              ...
              {upcoming.length > 0 ? upcoming.map((p, idx) => (
                <div key={idx}>
                  <div className="text-sm font-semibold text-white">
                    <span className="text-teal-400 mr-2">{formatTime(p.start)} - {formatTime(p.stop)}</span>
                    {p.title}
                  </div>
  ...
  return (
    <div
      ...
      className={`group flex items-center justify-between px-4 py-2.5 cursor-pointer transition-colors ${
        active ? 'bg-teal-700/80 text-white' : hidden ? 'text-gray-600 hover:bg-white/5' : 'text-gray-300 hover:bg-white/5'
      } ${focused ? 'ring-2 ring-inset ring-teal-300' : ''}`}
    >
      ...
      <span className="flex items-center gap-2 shrink-0">
        {onToggleHide && (
          <button
            ...
            className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-white/10 text-gray-400 transition-opacity"
          >
            {hidden ? <LucideEye size={13} /> : <LucideEyeOff size={13} />}
          </button>
        )}
        <span className={`text-xs tabular-nums ${active ? 'text-teal-100' : 'text-gray-500'}`}>{count}</span>
      </span>
    </div>
  );
}
    ...
    rows.push(
      <div
        ...
        className={`flex items-center px-3 gap-3 cursor-pointer border-b border-white/5 transition-colors ${
          isPlaying ? 'bg-teal-900/40 border-l-2 border-l-teal-400' : 'hover:bg-white/5 border-l-2 border-l-transparent'
        } ${isFocused ? 'ring-2 ring-inset ring-teal-300 bg-white/5' : ''}`}
      >
        <span className="w-10 shrink-0 text-right text-xs font-mono text-gray-500 tabular-nums">{channelNumbers.get(c.id) || ''}</span>
        ...
        <div className="flex-1 min-w-0">
          <div className={`text-sm font-semibold truncate ${isPlaying ? 'text-teal-300' : 'text-gray-100'}`}>{c.name}</div>
          <div className="text-xs text-gray-400 truncate">
            {now ? now.title : (c.groups?.[0] || '')}
          </div>
          {progress != null && (
            <div className="w-32 h-0.5 bg-white/10 rounded-full mt-1 overflow-hidden">
              <div className="h-full bg-amber-400 rounded-full" style={{ width: `${Math.round(progress * 100)}%` }} />
            </div>
          )}
        </div>
        <button
